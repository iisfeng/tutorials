package com.testeru.hcrscases.num;
...
public class HamcrestNumberUnitTest {
     ...
     */
    /**
     * Order Matchers With Integer Values
     * 最常见的情况是将这些匹配器与数字一起使用。
     * 所以，让我们继续创建一些测试
     */
    /**comparesEqualTo
     ...
     * 参数：
     * value – 当传递给被检查对象的 compareTo 方法时，应该返回零的值
     */
    @Test
     ...
     * 断言（2，大于（1））
     * 参数：
     * value – 当传递给被检查对象的 compareTo 方法时，应该返回大于零的值
     */
    @Test
     ...
     * 参数：
     * value – 当传递给被检查对象的 compareTo 方法时，应该返回大于或等于零的值
     */
    @Test
     ...
     * 参数：
     * value – 当传递给被检查对象的 compareTo 方法时，应该返回小于零的值
     */
    @Test
     ...
     *创建 Comparable 对象的匹配器，当检查对象小于或等于指定值时匹配，如检查对象的 compareTo 方法报告的那样。例如：
     ...
     参数：
     value – 当传递给被检查对象的 compareTo 方法时，应该返回小于或等于零的值
     */
    @Test
     ...
     * 尽管比较数字完全有意义，但很多时候比较其他类型的元素很有用。这就是订单匹配器可以应用于任何实现Comparable接口的类的原因
     * String在Comparable接口的compareTo方法中实现字母顺序。
     *
     *
     */
    @Test
     ...
     * 具有LocalDate值的订单匹配器
     * 与Strings 一样，我们可以比较日期。让我们看看上面创建的相同示例，但使用LocalDate对象：
     */
